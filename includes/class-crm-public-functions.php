...
{
    return' <br>
        <div class="powered_wsdesk"><span>Email is a service from '. get_bloginfo('name').'.</span><span> Powered by </span><a href="https://wsdesk.com" target="_blank">WSDesk</a></div>
        ';
}
...
function eh_crm_get_ticketmeta_value_count($key, $value,$order = 'ticket_id', $type = 'DESC', $limit = 0,$offset=0) {
    ...
    $data = $wpdb->get_results($wpdb->prepare("SELECT ticket_id,meta_value FROM $table WHERE " . $where, $key), ARRAY_A);
    if (!$data) {
        return array();
    } else {
        for ($i = 0; $i < count($data); $i++) {
